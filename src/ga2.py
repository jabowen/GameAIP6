...
class Individual_Grid(object):
    ...
    #put swap all the floating pipe segments with the non_pipe segment below it
    def fix_floating_pipes(self, genome):
        pipeOptions = ["|","T"]
        for x in range(left, right):
            last_non_pipe = height-1
            for y in range(height-1, -1, -1):
                if genome[y][x] in pipeOptions:
                    genome[y][x] = genome[last_non_pipe][x]
                    genome[last_non_pipe][x] = '|'
                    last_non_pipe-=1
            if last_non_pipe != height-1 and last_non_pipe != 0:
                genome[y][x] = 'T'
        return genome
    ...
    def mutate(self, genome):
        ...
        return fix_floating_pipes(genome)
